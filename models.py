...
from typing import Dict, Any, Callable, overload, TypeVar
...
class ChatModelWithName(BaseChatModel):
    ...
    def set_log_fn(self, log_fn: Callable[[float, str], None], function_name: str) -> None:
        self.log_fn = log_fn
    ...
    @overload
    async def ainvoke(self, messages: list[BaseMessage], output_format: None = None) -> Any: ...

    @overload
    async def ainvoke(self, messages: list[BaseMessage], output_format: type[T]) -> Any: ...
    ...
    def _serialize_message(self, msg: Any) -> BaseMessage:
